...
export default function FridgesDashboard() {
  ...
  const handleDelete = async (fridgeId: string) => {
    await submit("/api/fridges", { fridgeId }, {
      method: "DELETE",
      auth: true,
      successMessage: "Fridge deleted!",
      errorMessage: "Can't delete fridge!",
    });
    refetch();
  };
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
          ...
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              {rows.length === 0 ? "Get started by adding your first fridge!" : `You have ${rows.length} fridge${rows.length !== 1 ? "s" : ""}`}
            </Typography>
          </Box>
        </Box>
      ...
      {loading ? (
        ...
        <Paper sx={{ borderRadius: 4, py: 8, textAlign: "center" }}>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
            Add your first fridge and start tracking your food!
          </Typography>
        ...
        <Paper sx={{ borderRadius: 3, overflow: "hidden" }}>
          <DataGrid
            ...
            onRowClick={(params: GridRowParams) => navigate(`/fridgeitems/${params.row.id}`)}
